...
3. **`H_room_no_overlap` & Pool of Rooms (Кабинеты и Фоллбэк):**
   В кабинете $R$ не может проводиться более одного занятия в один и тот же слот $(d, p)$.
   Для предметов со спецкабинетом (`required_room_type`) алгоритм строит **Pool of Rooms**: выбирает профильный кабинет, а при его загруженности автоматически использует любой свободный `General` кабинет или базовый кабинет учителя (`base_room_id`), предотвращая сбои `INFEASIBLE`.
...
5. **`H_subject_domains` & Спаренные Уроки (Спарки):**
   * Для сдвоенных предметов (`is_double_allowed` / лабораторные): при установке 2 уроков в день они идут **строго последовательными соседними уроками** ($p$ и $p+1$).
   * При суммарной нагрузке предмета/домена $\le 5$ часов в неделю $\rightarrow$ **строго не более 1 урока в день** в данном классе.

6. **`H_no_student_windows` (АБСОЛЮТНЫЙ ЗАПРЕТ ОКОН У УЧЕНИКОВ ДЛЯ ВСЕХ КЛАССОВ):**
   Ни у одного класса в школе (1–11 классы) **не может быть «окон» (пустых уроков)** между первым и последним уроком дня. Все уроки дня образуют единый непрерывный блок.

7. **`H_math_ordering` (Порядок Старта Математического Блока):**
...
8. **`H_no_late_math` (Запрет Сложной Математики на Поздних Уроках):**
...
9. **`H_luo_priority` (Приоритет Спецклассов ЛУО/ДО):**
