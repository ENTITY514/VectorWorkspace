...
for(const f of files){
  ...
  for(let i=headerIdx+1;i<rows.length;i++){
    ...
    if(hasPeriod) currentPeriod=pn;
    const period=hasPeriod ? pn : currentPeriod;
    for(let k=0;k<dayCols.length;k++){
      const dc=dayCols[k]; const day=dayNames[k];
      const txt=norm(row[dc]);
      if(!txt) continue;
      const raw=row[dc].toString();
      const lines=raw.split('\n').map(x=>x.replace(/\s+/g,' ').trim()).filter(x=>x);
      const subject=lines[0]||'';
      const teacher=lines[1]||'';
      const time=lines[2]||'';
      const note=lines.slice(3).join('; ');
      lessons.push({class:cls, file:f, day, period, subject, teacher, time, note, raw});
      allLessons.push({class:cls, file:f, day, period, subject, teacher, time, note});
  ...
  // print detailed lessons sorted by period/day
  // group by day+period?
  for(const l of lessons){
    console.log(`  ${l.day} ур.${l.period}: "${l.subject}" — ${l.teacher} — ${l.time} — [${l.note}]`);
  }
}
...
console.log('\nКлассы:');
for(const c of classList) console.log(`  ${c.class} (${c.file}) — ${c.lessons} уроков — ${c.dateRange}`);
...
// per teacher lesson counts
const cntTeacher={};
for(const l of allLessons) cntTeacher[l.teacher]=(cntTeacher[l.teacher]||0)+1;
console.log('\nНагрузка по учителям:');
for(const [t,c] of Object.entries(cntTeacher).sort((a,b)=>b[1]-a[1])) console.log(`  ${t}: ${c}`);

// per class
const cntClass={};
for(const l of allLessons) cntClass[l.class]=(cntClass[l.class]||0)+1;
console.log('\nНагрузка по классам:');
for(const [c,n] of Object.entries(cntClass).sort()) console.log(`  ${c}: ${n}`);
...
console.log('\nSaved data/q4_2026_verification.json');
