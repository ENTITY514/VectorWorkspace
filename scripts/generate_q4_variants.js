...
const curriculum=[...curriculumMap.values()];
console.log(`Curriculum entries: ${curriculum.length}`);
fs.writeFileSync(path.join(root,'data','synthetic','curriculum_q4_2026_variant.json'), JSON.stringify(curriculum,null,2)+'\n','utf8');

// Generate Variant2: heuristic autogenerated respecting hard constraints (greedy)
function generateVariant2(slotsV1, curriculum){
  // For each class, we need to place curriculum hours across week.
  // We'll create empty occupancy maps.
  const teacherBusy = new Map(); // teacherId -> Set of "day|period"
  const classBusy = new Map(); // classId -> Set
  const roomBusy = new Map(); // roomId -> Set
  const result=[];

  // Sort curriculum by teacher load descending for better placement
  ...
  // Build room pool per required type
  ...
  // Subject required type map
  const subjReq=new Map(catalog.subjects.map(s=>[s.id, s.required_room_type]));

  // To vary from Variant1, we shuffle day/period order pseudo-random with seed
  let seed=42;
  ...
  const allDays=[0,1,2,3,4,5]; // 6 days for solver capacity
  const allPeriods=[0,1,2,3,4,5,6];
...
const slotsV2=generateVariant2(slotsV1, curriculum);
