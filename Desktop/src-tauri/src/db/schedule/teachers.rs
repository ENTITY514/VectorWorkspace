...
pub async fn list_teachers(pool: &SqlitePool) -> Result<Vec<ScheduleTeacher>, DbError> {
    let rows = sqlx::query_as::<_, (String, String, Option<String>, i64, String)>(
        "SELECT id, full_name, base_room_id, max_daily_lessons, availability_json FROM schedule_teachers ORDER BY full_name",
    )
    .fetch_all(pool)
    .await?;
    Ok(rows
        .into_iter()
        .map(|(id, full_name, base_room_id, max_daily_lessons, availability_json)| ScheduleTeacher {
            id,
            full_name,
            base_room_id,
            max_daily_lessons,
...
pub async fn upsert_teacher(
    ...
    sqlx::query(
        "INSERT INTO schedule_teachers (id, full_name, base_room_id, max_daily_lessons, availability_json)
         VALUES (?1, ?2, ?3, ?4, ?5)
         ON CONFLICT(id) DO UPDATE SET full_name=?2, base_room_id=?3, max_daily_lessons=?4, availability_json=?5",
    )
    .bind(&tid)
    .bind(&full_name)
    .bind(&base_room_id)
