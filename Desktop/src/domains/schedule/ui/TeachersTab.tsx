...
export function TeachersTab() {
  ...
  return (
    <div className="card">
      ...
      <ul>
        {filtered.map(t => <li key={t.id}>{t.full_name} · макс. {t.max_daily_lessons} ур./день · <button className="btn btn-small" onClick={() => scheduleApi.deleteTeacher(t.id).then(load)}>Удалить</button></li>)}
      </ul>
