...
import { WEIGHT_LABELS, WEIGHT_TOOLTIPS } from "../../../types";

export function WeightsTab({ state, onSaved }: { state: ScheduleState; onSaved: () => void }) {
  const [w, setW] = useState(state.weights);
  const save = async () => {
    await scheduleApi.setWeights({
      window: w.window,
      room_displacement: w.room_displacement,
      sanpin_parabola: w.sanpin_parabola,
      alternation: w.alternation,
      movement: w.movement,
      load_balance: w.load_balance,
      change_slot: w.change_slot,
    });
    onSaved();
  };

  const slider = (key: keyof ScheduleWeights, label: string) => {
    const tooltip = WEIGHT_TOOLTIPS[key];
    return (
      <div className="slider-row" key={key}>
        <label title={tooltip}>
          {label} {w[key] === 0 && <span className="badge">Отключено</span>}
          {tooltip && <span className="slider-hint">ℹ</span>}
        </label>
  ...
  return (
    <div className="card">
      ...
      {slider("change_slot", WEIGHT_LABELS.change_slot)}
